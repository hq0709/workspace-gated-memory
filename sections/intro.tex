...
above the competing candidates (AUC $0.63$--$0.67$ at 7B across three model
families), is not explained by surface presence, embedding relevance, or
...
Downstream, gating memory on the workspace score costs zero probe passes,
matches the canonical 1--10 rating everywhere the two can be compared, and
matches the selection oracle at 7B on the construct-valid benchmark ($0.525$
vs.\ $0.525$; $p{=}0.0013$ against the binary probe;
Fig.~\ref{fig:downstream}); against that probe it also raises 0.5B recall-QA
from $0.45$ to $0.75$ on Evoked ($p{<}10^{-4}$). Full comparisons are in
\S\ref{sec:prgresults}.
Finally, the
failure is repairable: the reporter never learned to read the workspace, and 500
steps of fine-tuning on labels derived from the model's own workspace
ranking, with no human annotation, lifts held-out verbal calibration from
${\approx}0.3$ to $0.62$--$0.65$ AUC on average across seeds while leaving
the workspace channel and QA accuracy statistically unchanged
(Table~\ref{tab:metacog}).
...
three model families and three probe wordings, with episode-cluster bootstrap
...
\begin{figure}[t]
\centering
\IfFileExists{figures/fig1_hero.pdf}{%
...
3B--7B in all three families, with knife-edge small-scale cells noted in
App.~\ref{app:prereg};
