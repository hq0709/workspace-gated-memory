...
($\Vrep$ up to $0.681$ at 7B; $\Vrep$ exceeds $\Wrr$ in all three families,
significantly in two under cluster CIs, and embedding relevance also beats
$\Wrr$ there): when importance is written on the surface, asking the model
...
distractors, while $\Wrr$ reads $0.53$--$0.67$ on Decoupled (at chance
on Compositional for two of three families, \S\ref{sec:boundary}); on the
...
App.~\ref{app:prereg}), in all three model families, and for all three
...
readout. In $\Ms$ terms: in the explicit regime the verbal channel dominates the
workspace channel outright ($\Vrep{>}\Wrr$ significantly), while on the
...
(orange box) in mid-to-late layers. The readout is measuring where the model
...
The signal is not a surface artifact. Figure~\ref{fig:mechanism} localizes it:
the inferred bridge becomes decodable specifically at the context token that
licenses the inference, in mid-to-late layers, rather than uniformly across
the passage or at the concept's own mention, which never occurs. This is the
readout reading a context-specific computation, consistent with its
above-chance recovery where surface and embedding relevance are at chance
(Table~\ref{tab:master}) and with the causal test of \S\ref{sec:causal}.
...
\begin{tabular}{lcccc}
\toprule
 & \multicolumn{2}{c}{0.5B} & \multicolumn{2}{c}{7B} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
Policy & $k{=}2$ & $k{=}3$ & $k{=}2$ & $k{=}3$ \\
\midrule
