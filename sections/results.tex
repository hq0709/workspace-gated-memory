...
($\Vrep$ up to $0.681$ at 7B; $\Vrep$ exceeds $\Wrr$ in three of four
families, significantly in two under cluster CIs, and sits at chance alongside
$\Wrr$ in the fourth (Qwen3-8B), where the explicit items are simply less
salient to both channels; embedding relevance also beats $\Wrr$ where $\Vrep$ does): when importance is written on the surface, asking the model
...
\S\ref{sec:boundary}) $\Vrep$ drops to $0.25$--$0.49$ on Decoupled ($0.17$ on Compositional), at
or below chance, with confident yes-ratings concentrated on vivid
distractors, while $\Wrr$ reads $0.53$--$0.67$ on Decoupled (at or near
...
(Table~\ref{tab:master}). The paired difference is
significant at three of four sizes on the original construct-valid Decoupled
set and at 1.5B on the 120-episode scaled set built for that knife-edge case
(Bonferroni-corrected cluster CIs, both intervals reported in
App.~\ref{app:prereg}), in all four model families, and for all three
probe wordings (App.~\ref{app:vrobust}). Across
three benchmark generators the two channels differ in kind: the workspace
readout stays within a narrow band ($0.53$--$0.66$ on the Decoupled design)
...
readout. In $\Ms$ terms: in the explicit regime the verbal channel matches or beats the
workspace channel in three of four families (both at chance in Qwen3-8B),
significantly in two, while on the
construct-valid benchmarks $\Ms$ goes negative at every scale where it is
defined: the report ranks the truly useful items below the rest. Embedding
...
The dissociation is not a fixed-scale curiosity but sharpens with model
capability. Table~\ref{tab:q3scale} traces it across the newest Qwen3 family:
on silently inferred content the workspace$-$verbal gap is significantly
positive at both capable scales (4B and 8B, in every inferred regime) and
reaches $+0.317$ on Decoupled at 8B, on par with the strongest dissociation we
measure in any family (Table~\ref{tab:master}),
while on explicit content it is null or favors the verbal report (significantly
so at 1.7B). The verbal channel does not improve with scale on inferred content
--- it stays anti-calibrated, below chance --- even as the workspace readout
climbs: the larger model carries more of the silent inference in its state yet
reports it no better, which is exactly the regime where a workspace gate pays
off.

\input{tables/tab_q3scale}
...
Workspace (zero passes) & 0.332 & 0.431 & 0.502 & \textbf{0.636} \\
...
score wins here. Three results follow. First, replacing the
...
content-scoring policy in any cell. Second, the failure is specific to the verbal channel, not to internal
signals generally: against the canonical 1--10
...
\S\ref{sec:causal}.
Third, at the tight 7B budget provenance routing is the strongest policy
